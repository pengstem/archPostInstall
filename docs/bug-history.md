...
- GNOME screensaver shortcut: `Super+F11` was registered and `gsd-media-keys` launched the configured command, but the launcher failed with `required command not found: tte`. The current TTE runtime was installed by uv at `~/.local/bin/tte`, while the GNOME/systemd session `PATH` omitted `~/.local/bin`.
  ...
  Superseded later that day by the Rust-only `ttfx` renderer; the Python runtime and fallback path were removed.

## 2026-08-28
- Fortune/cowsay greeting vs Powerlevel10k instant prompt: `fortune | cowsay` printed at source time in `85-fortune-cowsay.zsh` triggered p10k's "console output during zsh initialization" warning on every start.
  First attempted fix (deferred to a one-shot `precmd` hook) passed plain-pty tests but still warned in real kitty windows: probe logging showed the hook ran while fd 1 still pointed at p10k's capture temp file. Root cause: `_p9k_precmd` re-orders itself to the END of `precmd_functions` (and prepends `_p9k_do_nothing`), while fd restoration happens inside `_p9k_clear_instant_prompt`; whether a custom hook lands before or after the restore is an environment-dependent lottery (wezterm/kitty integration hooks, instant-prompt cache presence). So any custom precmd greeting can still be captured.
  Final fix: embrace the capture — print at source time and set `POWERLEVEL9K_INSTANT_PROMPT=quiet`. p10k buffers init-time output and unconditionally replays it above the first prompt (`command cat` of the capture file in `_p9k_clear_instant_prompt`), so nothing is lost; `quiet` only skips the warning. The setting must live in `configs/p10k.zsh` (~line 1813): the p10k-configure-generated file is sourced at `40-tool-init.zsh` and overwrites anything set earlier — setting `quiet` in `00-instant-prompt.zsh` has no effect. Caveat: re-running `p10k configure` regenerates that file back to `verbose`; re-check the option after any configure run. Verified in three sequential real kitty windows (`kitty -e script -qfec "zsh -l -i"`, warning count 0, cow replayed above the prompt) with Zsh 5.9.2 and Powerlevel10k 1.20.17 on 2026-08-28. Startup cost: `zsh -i -c exit` went from 157.5 ms to 182.1 ms (~25 ms for the fortune|cowsay pipeline now sitting in the init path; previously deferred, it was paid just after init instead).

## 2026-09-24
- Fresh-install bootstrap: `bootstrap.sh` installed packages before `setup.sh` linked the tracked `/etc/pacman.conf`, but the package list needs `[multilib]` (`steam`), which only that config enables. The config also `Include`d `/etc/pacman.d/openai-chatgpt.conf`, which only exists after that vendor package configures itself, so pacman would refuse to read it on a new machine.
  Fix: bootstrap installs the `pacman` manifest group first and runs a full `pacman -Syu` (not `-Sy`, to avoid a partial upgrade); `setup.sh` renders `/etc/pacman.conf` with `Include` lines for missing files commented out.
- Root-executed files in the user-writable repo: `/etc/pacman.conf`, `/etc/paru.conf`, the pacman hook, TLP, the mkinitcpio preset/drop-in, and the kernel cmdline were symlinks into `~/Project/archPostInstall`, and the pkglist hook ran `scripts/update_pkglist.sh` as root on every transaction. Any code running as the user could edit those files to get root on the next `pacman`/`mkinitcpio` run.
  Fix: `manifest.tsv` installs them as root-owned copies (`archpostinstall sync-system` after edits); the hook runs a root-owned wrapper that drops to the repo owner with `runuser`, and `update_pkglist.sh` refuses to run as root. `install_boot_splash.sh` copies and verifies the mkinitcpio/cmdline files instead of symlinking them.
