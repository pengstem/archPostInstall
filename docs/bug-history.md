...
## 2026-08-28
- Fortune/cowsay greeting: printing `fortune | cowsay` at source time in `85-fortune-cowsay.zsh` emitted console output during zsh initialization, which Powerlevel10k's instant prompt flags with its "console output during zsh initialization" warning on every interactive start.
  Fix: move the greeting into a one-shot `precmd` hook that removes itself before printing, so the greeting appears above the first real prompt with zero console I/O during initialization. Verified in a pty session (warning count 0, greeting printed once, hook removed after first run) with Zsh 5.9.2 and Powerlevel10k on 2026-08-28.
